...
static int cpufreq_governor_interactive1(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVE1
static
#endif
struct cpufreq_governor cpufreq_gov_interactive1 = {
	.name = "interactive1",
	.governor = cpufreq_governor_interactive1,
 ...
 */
static unsigned int cpufreq_interactive1_calc_freq(unsigned int cpu)
{
...
static void cpufreq_interactive1_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
	...
	for_each_cpu(cpu, tmp_mask) {
		if (target_freq == policy->max) {
			...
			target_freq = cpufreq_interactive1_calc_freq(cpu);
			__cpufreq_driver_target(policy, target_freq,
							CPUFREQ_RELATION_L);
		}
...
static struct attribute *interactive1_attributes[] = {
	&min_sample_time_attr.attr,
	NULL,
};

static struct attribute_group interactive1_attr_group = {
	.attrs = interactive1_attributes,
	.name = "interactive1",
};

static int cpufreq_governor_interactive1(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	int rc;
	switch (event) {
...
static int __init cpufreq_interactive1_init(void)
{
	unsigned int i;
	struct timer_list *t;
	...
	up_wq = create_rt_workqueue("kinteractive1_up");
	down_wq = create_workqueue("knteractive1_down");

	INIT_WORK(&freq_scale_work, cpufreq_interactive1_freq_change_time_work);

	return cpufreq_register_governor(&cpufreq_gov_interactive1);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVE1
pure_initcall(cpufreq_interactive1_init);
#else
module_init(cpufreq_interactive1_init);
#endif

static void __exit cpufreq_interactive1_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_interactive1);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_interactive1_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_interactive1' - A cpufreq governor for "
